...
function stepHit(curStep){
	if (curStep >= 272 && curStep <= 1304) {
		if (curStep % 8 == 0){
			...
			FlxTween.tween(cpuStrums.members[i], {y: defaultStrumY}, 0.65, {ease: FlxEase.backOut});}
			for (i in 0...4){
			FlxTween.globalManager.completeTweensOf(playerStrums);
			playerStrums.members[i].y+=20;
			FlxTween.tween(playerStrums.members[i], {y: defaultStrumY}, 0.65, {ease: FlxEase.backOut});}
		    }
		}
